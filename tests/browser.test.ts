...
import { BrowserService } from "../apps/server/src/browser/browser.ts";
import { Auth } from "../apps/server/src/platform/auth.ts";
import { Files } from "../apps/server/src/platform/files.ts";
import { capturePdfDownload, readDownloadFailures } from "../apps/worker/src/browser/downloads.ts";
import { isPublicIp, validatePublicUrl } from "../apps/worker/src/browser/network.ts";
import { startEgressProxy } from "../apps/worker/src/browser/proxy.ts";
...
test("browser API reopens an owned profile at the edited address and renews console access", async (t) => {
  ...
  assert.deepEqual(calls[0], {
    path: "/sessions",
    // A tab of the person's one profile (here the only session is that profile).
    body: { id: sessionId, url: "https://example.org/", profile: sessionId, adopt: [] },
  });
  ...
  assert.match(await console.text(), /Texto para digitar no navegador/);
...
test("WebSockets follow the same public-destination rule as pages", async () => {
  const { socketHttpUrl } = await import("../apps/worker/src/browser/browser.ts");
  assert.equal(socketHttpUrl("wss://web.whatsapp.com/ws/chat"), "https://web.whatsapp.com/ws/chat");
  assert.equal(socketHttpUrl("ws://example.com/live"), "http://example.com/live");
  const publicHost = async () => [{ address: "93.184.216.34", family: 4 }];
  await validatePublicUrl(socketHttpUrl("wss://web.whatsapp.com/ws"), publicHost);
  for (const url of ["ws://127.0.0.1:8787/api", "wss://169.254.169.254/", "wss://localhost/x"])
    await assert.rejects(validatePublicUrl(socketHttpUrl(url)), { code: "BLOCKED_URL" }, url);
});

test("each chat and task gets its own tab of the person's one browser, and old sign-ins come along", async (t) => {
  const opened: Record<string, unknown>[] = [];
  const { db, service } = await browserFixture(t, (path, body) => {
    if (path === "/sessions") opened.push(body);
    return path.endsWith("/read")
      ? { data: { url: "https://x.org", title: "x", text: "x", truncated: false } }
      : { data: { ...savedSession, id: body.id ?? savedSession.id, url: String(body.url ?? "") } };
  });
  // Before tabs: a browser the person signed in to (it becomes the profile) and another one.
  const signedIn = "11111111-1111-4111-8111-111111111111";
  ...
  const one = await service.observeForThread(
    "owner",
    "thread-a",
    "https://www.mercadolivre.com.br/",
  );
  const two = await service.observeForThread(
    "owner",
    "task-thread-b",
    "https://www.amazon.com.br/",
  );
  assert.notEqual(one.sessionId, two.sessionId, "separate tabs work in parallel");
  assert.deepEqual(
    opened.map((body) => body.profile),
    [signedIn, signedIn],
    "both are tabs of the signed-in profile",
  );
  const adopted = (opened[0]?.adopt ?? []) as string[];
  assert.ok(adopted.includes(older), "older sign-ins are merged in");
  assert.ok(!adopted.includes(signedIn));
  // Signing in on a tab keeps the same profile.
  await service.login("owner", one.sessionId).catch(() => undefined);
  await service.observeForThread("owner", "thread-c", "https://www.ifood.com.br/");
  assert.equal(opened.at(-1)?.profile, signedIn);
});
