...
test("missing browser setup is explicit rather than a fictional browser session", async () => {
  ...
  assert.match((await response.json()).error, /não está configurado/);
});
...
test("clients cannot propose a checkout directly; only the agent can", async () => {
  const response = await app.request("/api/actions", {
    ...
    body: JSON.stringify({
      ...
      data: {
        ...
        url: "https://www.ifood.com.br/pedido",
        total: "R$ 10,00",
        totalValue: 10,
        button: "Fazer pedido",
        summary: "Teste",
  ...
  assert.match(JSON.stringify(await response.json()), /propostos pelo agente/);
});
