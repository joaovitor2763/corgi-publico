...
import { GoogleAuth } from "../apps/server/src/workspace/google-auth.ts";
...
test("old refresh cannot overwrite a newly connected Google account", async (t) => {
  ...
  await assert.rejects(refresh, /conta .* mudou/i);
  assert.equal((await auth.tokens("owner"))?.account, "b@example.com");
  await assert.rejects(auth.accessToken("owner", "connection-a"), /conexão .* mudou/i);
});
...
test("OAuth callbacks require a known, single-use state", async (t) => {
  ...
  const auth = new GoogleAuth(db, config);
  await assert.rejects(auth.callback("unknown-state", "untrusted-code"), /expirou/);
  ...
  await assert.rejects(auth.callback("expired", "untrusted-code"), /expirou/);
...
for (const stage of ["token", "profile"]) {
  test(`disconnect invalidates an OAuth callback suspended during ${stage}`, async (t) => {
    ...
    release.resolve();
    await assert.rejects(callback, /desconectad|expirou|mudou/i);
...
test("disconnect invalidates pending OAuth state before a callback contacts Google", async (t) => {
  ...
  await assert.rejects(auth.callback(state, "synthetic-code"), /desconectad|expirou|mudou/i);
...
test("a newer connect attempt invalidates an older callback already exchanging its code", async (t) => {
  ...
  release.resolve();
  await assert.rejects(oldCallback, /desconectad|expirou|mudou/i);
...
test("an in-flight refresh cannot restore credentials after disconnect", async (t) => {
  ...
  release.resolve();
  await assert.rejects(refresh, /desconectad/i);
