...
for (const backend of ["model", "pi"] as const)
  test(`${backend} worker executes server tools and persists the confirmed outcome`, async (t) => {
    ...
    const fixture =
      backend === "pi"
        ? await piModelFixture(t, (index) => ({ calls: calls[index] ? [calls[index]] : [] }))
    ...
    const server = await createApp(
      db,
      {
        ...
        chatTasks: false,
        model: backend === "pi" ? undefined : "openai/fixture",
        impossiblBaseUrl:
          "baseUrl" in fixture && typeof fixture.baseUrl === "string" ? fixture.baseUrl : undefined,
    ...
    try {
      ...
      assert.ok(result.events.some((event) => event.title === "Lendo seu espaço de trabalho"));
      assert.ok(requests.length >= 4 && requests.length <= 6);
      assert.ok(
        requests.every(
          (request) =>
            request.path === (backend === "pi" ? "/v1/chat/completions" : "/v1/responses"),
        ),
      );
      ...
      assert.equal(
        (await db.list<ActionProposal>("owner", "actions")).filter(
          (a) => a.taskId === appointment.id,
        ).length,
        1,
      );
    } finally {
