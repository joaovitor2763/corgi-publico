...
import { BrowserThreadCard } from "../computer/computer";
...
export function FileThreadCard({ file }: { file: Artifact }) {
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={`Abrir PDF: ${file.name}`}
      ...
      <Card style={{ padding: 18, backgroundColor: "#F0F1F2", gap: 18 }}>
        <View style={{ borderRadius: 12, padding: 22, backgroundColor: "#FFF", gap: 14 }}>
          ...
          {file.fields?.length ? (
            ...
            <Text style={s.muted}>
              {file.pageCount} {file.pageCount === 1 ? "página" : "páginas"} · Toque para ler o
              documento
            </Text>
          )}
        </View>
    ...
    <View style={{ gap: 12 }}>
      ...
      {error && (
        <Button small onPress={() => setAttempt((value) => value + 1)}>
          Recarregar resultados da tarefa
        </Button>
      )}
    </View>
  );
