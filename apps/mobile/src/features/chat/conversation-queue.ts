...
export class ConversationQueue {
  ...
  /** Messages saved from a previous session go ahead of anything typed since. */
  restore(messages: readonly QueuedMessage[]) {
    const known = new Set(this.state.pending.map((message) => message.id));
    const fresh = messages.filter((message) => !known.has(message.id));
    if (fresh.length) this.update({ pending: [...fresh, ...this.state.pending] });
  }
  remove(id: string) {
