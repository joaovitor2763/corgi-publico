...
import { isCompleteInstant, localDateTime, zonedInstant } from "../src/shared/date-time.ts";
...
test("a daylight-saving gap cannot become a different appointment time", () => {
  assert.throws(() => zonedInstant("2026-03-08", "02:30", "America/Los_Angeles"), /não existe/);
  assert.equal(
