...
import { ConversationQueue } from "../src/features/chat/conversation-queue.ts";
...
test("messages saved from a previous session go first and are never duplicated", () => {
  const queue = new ConversationQueue();
  queue.enqueue({ id: "new", text: "digitada agora" });
  queue.restore([
    { id: "old", text: "ficou na fila" },
    { id: "new", text: "digitada agora" },
  ]);
  assert.deepEqual(
    queue.getSnapshot().pending.map((m) => m.id),
    ["old", "new"],
