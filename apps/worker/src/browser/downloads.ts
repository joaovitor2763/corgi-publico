...
export async function capturePdfDownload(options: {
  ...
  const outcome: Outcome = {
    ...
    message: "O download foi interrompido ou não pôde ser recuperado. Tente baixar de novo.",
  ...
  try {
    if (limitReached) {
      ...
      outcome.message = "Esta sessão atingiu o limite de 20 downloads de PDF.";
    ...
    if (prefix.toString("ascii") !== "%PDF-") {
      ...
      outcome.message = "Só é possível importar downloads em PDF. Este arquivo não era um PDF.";
    ...
    if (oversized) {
      ...
      outcome.message = "O download ultrapassa 10 MiB. Escolha um PDF menor.";
    }
    outcome.status = "failed";
    await persist(outcomePath, outcome);
