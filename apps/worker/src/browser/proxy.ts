...
export async function startEgressProxy() {
  ...
  server.on("connect", async (request, client, head) => {
    ...
    try {
      ...
      // Idle long enough for a live connection (WhatsApp Web pings every ~30 s, some every few
      // minutes) without keeping dead tunnels forever.
      upstream.setTimeout(5 * 60_000, () => upstream.destroy());
      upstream.on("close", () => {
