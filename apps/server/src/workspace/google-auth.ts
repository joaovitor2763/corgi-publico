...
export class GoogleAuth {
  ...
  private decodeTokens(stored: Credential | null): Tokens | null {
    ...
    if (!this.config.encryptionKey)
      throw new AppError("TOKEN_ENCRYPTION_KEY não está configurada", 503);
  ...
  private async save(owner: string, tokens: Tokens, generation: string) {
    ...
    if (!saved)
      throw new AppError("O login do Google mudou ou foi desconectado. Conecte de novo.", 409);
  }
  ...
  async connect(owner: string, write: boolean) {
    if (!this.configured())
      throw new AppError(
        "Configure GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET e TOKEN_ENCRYPTION_KEY para conectar o Google",
        503,
      );
  ...
  async callback(stateId: string, code: string) {
    ...
    if (!state || state.expiresAt < Date.now())
      throw new AppError("O login do Google expirou. Conecte de novo.", 400);
    ...
    if (!response.ok)
      throw new AppError("O Google não conseguiu concluir o login. Conecte de novo.", 502);
    ...
    if (!profile.ok)
      throw new AppError("O Google não concedeu acesso de leitura ao Gmail. Conecte de novo.", 403);
  ...
  private async refresh(owner: string, tokens: Tokens) {
    if (!tokens.refreshToken)
      throw new AppError("A sessão do Google expirou. Conecte de novo.", 401);
    const response = await fetch("https://oauth2.googleapis.com/token", {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: new URLSearchParams({
    ...
    if (!response.ok) throw new AppError("A sessão do Google expirou. Conecte de novo.", 401);
    const token = tokenSchema.parse(await response.json());
    const refreshed = {
      ...tokens,
      accessToken: token.access_token,
    ...
    if (!this.config.encryptionKey)
      throw new AppError("A criptografia de token não está configurada", 503);
    ...
    if (!updated)
      throw new AppError("A conta do Google mudou ou foi desconectada durante a renovação", 409);
  ...
  async disconnect(owner: string) {
    ...
    if (tokens) {
      ...
      if (!response.ok && response.status !== 400)
        throw new AppError(
          "Desconectado localmente. A revogação no Google falhou; remova o acesso nas configurações da sua conta Google.",
          502,
        );
    }
  }
