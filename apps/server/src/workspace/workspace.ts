...
import { agentConfigured } from "../agent/runtime.ts";
...
export class WorkspaceService {
  ...
  async connection(owner: string) {
    if (this.config.mode === "sample" && this.config.sampleData === false) return null;
  ...
  async connected(owner: string) {
    if (this.config.mode === "sample" && this.config.sampleData === false) return false;
  ...
  async thread(owner: string, id: string) {
    const connection = await this.connection(owner);
    if (!connection) throw new AppError("O Google está desconectado", 409);
    const mail =
    ...
    if (!mail.length) throw new AppError("Conversa de e-mail não encontrada", 404);
  ...
  async ensureSample(owner: string, actions: ActionService) {
    if (this.config.mode !== "sample" || this.config.sampleData === false) return;
  ...
  /**
   * With SAMPLE_DATA=false, removes the fictional mailbox, calendar and the ideas and
   * pending reviews built from them. Conversations, tasks and memories are untouched.
   */
  async purgeSample(owner: string) {
    if (this.config.mode !== "sample" || this.config.sampleData !== false) return;
    if (!(await this.db.get(owner, "settings", "seeded"))) return;
    for (const kind of ["mail", "events", "ideas"])
      for (const item of await this.db.list<{ id: string }>(owner, kind))
        await this.db.remove(owner, kind, item.id);
    for (const action of await this.db.list<ActionProposal>(owner, "actions"))
      if (
        action.status === "awaiting_review" &&
        JSON.stringify(action.data).includes("@example.com")
      )
        await this.db.put(owner, "actions", { ...action, status: "cancelled" });
    await this.db.remove(owner, "settings", "seeded");
  }
  ...
  async snapshot(owner: string, query?: string): Promise<Workspace> {
    ...
    return {
      ...
      runtime: {
        ...
        sampleData: this.config.mode === "sample" && this.config.sampleData !== false,
      },
    };
  }
  async prepare(owner: string, input: ProposalInput, connectionId?: string) {
    if (input.kind === "app.action" || input.kind === "browser.checkout") return { input };
  ...
  async execute(
    ...
    if (input.kind === "app.action" || input.kind === "browser.checkout")
      throw new AppError("Ações de apps e checkouts não são executados pelo Google", 500);
    if (this.config.mode === "sample") {
      if (input.kind === "email.send") {
        const id = randomUUID();
    ...
    const tokens = await this.googleAuth.tokens(owner);
    if (!tokens) throw new AppError("O Google está desconectado", 409);
    ...
    if (!tokens.scopes.includes(`https://www.googleapis.com/auth/${capability}`))
      throw new AppError(
        "Habilite o acesso de escrita do Google em Conexões antes de aprovar",
        403,
      );
    if (tokens.connectionId !== connectionId)
      throw new AppError("A conta ou a conexão do Google mudou. Prepare uma nova ação.", 409);
    const google = this.google(owner, connectionId);
    if ((input.kind === "calendar.update" || input.kind === "calendar.delete") && !targetVersion)
      throw new AppError(
        "Essa revisão de calendário é anterior às checagens de versão. Prepare uma nova revisão.",
        409,
      );
    if (input.kind === "email.send") {
  ...
  async importAttachment(owner: string, reference: string): Promise<Artifact> {
    ...
    if (!messageId || !attachmentId || !filename)
      throw new AppError("Referência de anexo inválida");
    ...
    if (!message?.attachments.includes(reference) || message.connectionId !== connection.id)
      throw new AppError("Anexo não encontrado. Atualize a caixa de entrada da conta atual.", 404);
    const file = await this.files.import(
      owner,
