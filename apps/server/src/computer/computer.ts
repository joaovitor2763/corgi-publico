  ...
  new Promise((resolve) => {
    ...
    child.on("error", () => {
      capture(
        stderr,
        Buffer.from(
          "A CLI do Docker não conseguiu iniciar. Instale o Docker e ligue o engine dele.",
        ),
      );
      finish();
    });
...
export function workspacePath(path: string): string {
  if (
    ...
    throw new AppError("Escolha um caminho absoluto dentro de /workspace", 422);
...
export class ComputerService {
  ...
  private enabled() {
    if (!this.config.computerEnabled)
      throw new AppError(
        "Computer não está configurado. Ative COMPUTER_ENABLED e construa a imagem local.",
        503,
      );
  }
  private image() {
    ...
    if (!/^[a-zA-Z0-9][a-zA-Z0-9._/:@-]{0,250}$/.test(image))
      throw new AppError("COMPUTER_IMAGE é inválida", 503);
    return image;
  }
  private async checked(args: string[]) {
    const result = await this.docker(args, { timeoutMs: controlTimeout });
    if (result.timedOut)
      throw new AppError("Docker não respondeu em 10 segundos. Verifique o engine do Docker.", 503);
    if (result.interrupted || result.exitCode !== 0 || result.truncated)
      throw new AppError(
        "Operação do Docker falhou. Verifique se o engine está rodando e se a imagem do computer foi construída localmente.",
        503,
      );
  ...
  private async inspect(owner: string): Promise<Inspection | undefined> {
    ...
    if (!result.success)
      throw new AppError("Verificação de isolamento do computer falhou; conexão recusada", 409);
    ...
    if (!safe)
      throw new AppError(
        "Propriedade ou isolamento do computer não corresponde a este deployment; conexão recusada",
        409,
      );
  ...
  private async verifyVolume(owner: string) {
    ...
    if (!parsed.success)
      throw new AppError("Verificação de propriedade do workspace do computer falhou", 409);
    ...
    if (
      ...
      throw new AppError("Propriedade ou isolamento do workspace do computer não corresponde", 409);
  }
  private async acquire(owner: string, operation: Lease["operation"] = "operation") {
    ...
    if (previous && previous.expiresAt > Date.now())
      throw new AppError("Computer está ocupado. Espere a operação atual terminar.", 409);
        ...
        if (command.status === "running") {
          const saved = await this.db.compareAndSwap<ComputerCommand>(
            ...
            { status: "running" },
            {
              ...
              stderr:
                "A execução foi interrompida. O resultado é incerto; confira os arquivos antes de rodar de novo.",
            },
          );
  ...
  async snapshot(owner: string): Promise<ComputerSnapshot> {
    ...
    if (!base.enabled)
      return {
        ...
        status: "unconfigured",
        message:
          "Ative o computer Docker no servidor para usar o terminal e os arquivos do workspace.",
      };
    try {
      return {
        ...base,
      ...
      return {
        ...
        message:
          error instanceof AppError
            ? error.message
            : "Falha ao inspecionar o computer. Verifique a configuração do Docker.",
      };
    }
  }
  ...
  async stop(owner: string) {
    ...
    if (!lease || lease.expiresAt <= Date.now()) lease = await this.acquire(owner);
      ...
      throw new AppError(
        "Computer está ocupado com outra operação. Tente Parar de novo em instantes.",
        409,
      );
    ...
    if (!stopping) throw new AppError("Computer está ocupado com outro pedido de parada", 409);
    try {
        ...
        if (command.status === "running")
          await this.db.compareAndSwap(
            ...
            { status: "running" },
            {
              ...
              completedAt: new Date().toISOString(),
              stderr:
                "Interrompido pelo usuário. Confira o workspace antes de repetir este comando.",
            },
          );
  ...
  private async running(owner: string) {
    this.enabled();
    if (!(await this.inspect(owner))?.State.Running)
      throw new AppError("Inicie o computer antes de usar o terminal ou os arquivos", 409);
  ...
  async execute(
    ...
    if (previous) {
      if (previous.command !== args.command || previous.cwd !== cwd)
        throw new AppError("Este ID de operação já pertence a outro comando", 409);
    ...
    return this.exclusive(
      ...
      async (lease) => {
        ...
        if (options.signal?.aborted)
          throw new AppError("O comando do computer foi interrompido antes da execução", 409);
        ...
        if (!saved) {
          ...
          throw new AppError("Não foi possível salvar o recibo do computer", 500);
        }
        ...
        if (
          ...
          return this.db.put(owner, "computer-commands", {
            ...command,
            status: "interrupted",
            stderr: "Interrompido antes da execução",
            completedAt: new Date().toISOString(),
          });
        ...
        try {
          ...
          result = {
            stdout: "",
            stderr:
              "A execução no Docker foi interrompida; confira o workspace antes de tentar de novo.",
            exitCode: null,
            interrupted: true,
            timedOut: false,
            truncated: false,
        ...
        if (result.timedOut || result.interrupted) {
          ...
          if (cleanup) {
            try {
              ...
              result.stderr +=
                "\nNão foi possível confirmar a parada do container. O computer segue travado; tente Parar de novo depois de checar o Docker.";
            }
          }
        }
  ...
  private async file<T>(
    ...
    if (text !== undefined && Buffer.byteLength(text) > fileLimit)
      throw new AppError("Arquivos de texto devem ter no máximo 256 KB", 413);
    return this.exclusive(owner, async () => {
      ...
      if (result.exitCode !== 0 || result.timedOut || result.interrupted || result.truncated)
        throw new AppError(
          "Operação de arquivo do computer falhou. Verifique o caminho, as permissões e o tamanho; links simbólicos não podem ser abertos.",
          422,
        );
      try {
        return JSON.parse(result.stdout) as T;
      } catch {
        throw new AppError("Computer retornou uma resposta de arquivo inválida", 502);
      }
    });
  }
  ...
  async writePdf(owner: string, path: string, bytes: Uint8Array) {
    if (bytes.length > 10 * 1024 * 1024 || Buffer.from(bytes.subarray(0, 5)).toString() !== "%PDF-")
      throw new AppError("Escolha um PDF de até 10 MB", 422);
