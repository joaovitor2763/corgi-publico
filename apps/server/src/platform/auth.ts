...
export class Auth {
  ...
  async session(accessKey?: string) {
    // Live mode always needs the key; any other mode needs it as soon as one is configured
    // (e.g. local mode reached from a phone through Tailscale).
    const required = this.config.mode === "live" || !!this.config.accessKey;
    if (
      required &&
      (!accessKey ||
      ...
      throw new AppError("Chave de acesso incorreta", 401);
  ...
  async owner(authorization?: string) {
    if (!authorization?.startsWith("Bearer ")) throw new AppError("Faça login no OpenMuse", 401);
    ...
    if (!session || session.expiresAt < Date.now())
      throw new AppError("Sessão expirada. Faça login de novo.", 401);
  ...
  verify(url: URL) {
    ...
    if (
      ...
      throw new AppError("Link do documento expirou; atualize o workspace", 401);
    const expected = createHmac("sha256", this.signingKey)
      .update(`${owner}\n${url.pathname}\n${expires}`)
      .digest("hex");
    if (!timingSafeEqual(Buffer.from(expected), Buffer.from(signature)))
      throw new AppError("Link de acesso inválido", 403);
    return owner;
  }
}
